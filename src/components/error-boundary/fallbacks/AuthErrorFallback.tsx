...
export default function AuthErrorFallback({ message, onRetry, compact }: FallbackProps) {
  return (
    <div className={`flex flex-col items-center justify-center text-center ${compact ? 'p-4 gap-2' : 'p-8 gap-4'}`}>
      <ShieldAlert className={`text-amber-400/80 ${compact ? 'w-5 h-5' : 'w-10 h-10'}`} />
      <div>
        <p className={`font-semibold text-slate-200 ${compact ? 'text-xs' : 'text-base'}`}>인증 오류</p>
      ...
      <div className="flex items-center gap-2">
        ...
        {!compact && (
          <Link
            href="/dashboard"
            className="inline-flex items-center gap-1.5 px-4 py-2 rounded-lg bg-slate-800 hover:bg-slate-700 text-slate-300 text-sm transition-colors"
          >
