...
export default function NetworkErrorFallback({ message, onRetry, compact }: FallbackProps) {
  return (
    <div className={`flex flex-col items-center justify-center text-center ${compact ? 'p-4 gap-2' : 'p-8 gap-4'}`}>
      <WifiOff className={`text-red-400/80 ${compact ? 'w-5 h-5' : 'w-10 h-10'}`} />
      <div>
        <p className={`font-semibold text-slate-200 ${compact ? 'text-xs' : 'text-base'}`}>네트워크 오류</p>
