...
export default function UnknownErrorFallback({ message, onRetry, compact }: FallbackProps) {
  return (
    <div className={`flex flex-col items-center justify-center text-center ${compact ? 'p-4 gap-2' : 'p-8 gap-4'}`}>
      <AlertTriangle className={`text-red-400/70 ${compact ? 'w-5 h-5' : 'w-10 h-10'}`} />
      <div>
        <p className={`font-semibold text-slate-200 ${compact ? 'text-xs' : 'text-base'}`}>오류가 발생했습니다</p>
      ...
      <div className="flex items-center gap-2">
        {onRetry && (
          ...
          >
            <RotateCcw className={compact ? 'w-3 h-3' : 'w-4 h-4'} /> 재시도
          </button>
        )}
        <Link
          href="/dashboard"
