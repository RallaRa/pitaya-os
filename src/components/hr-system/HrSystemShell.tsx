...
import { Loader2, ChevronRight, UsersRound } from 'lucide-react';
...
import {
  HR_SYSTEM_MENU_SECTIONS,
  canAccessHrSystemSection,
  findHrSystemMenuItem,
  findHrSystemSection,
} from '@/lib/hr-system/menuStructure';
...
export default function HrSystemShell({ children, title, description, actions }: Props) {
  ...
  const currentItem = findHrSystemMenuItem(pathname);
  const currentSection = findHrSystemSection(pathname);
  const pageTitle = title || currentItem?.label || '인사/급여관리';
  ...
  useEffect(() => {
    ...
    const hasAny = HR_SYSTEM_MENU_SECTIONS.some(s =>
      canAccessHrSystemSection(menuAccess, s.permission),
    ) || menuAccess.hrSystem;
    if (!hasAny) router.replace('/dashboard');
  }, [loading, menuAccess, isSuperuser, hasModule, router]);

  const visibleSections = HR_SYSTEM_MENU_SECTIONS.filter(s =>
    isSuperuser || canAccessHrSystemSection(menuAccess, s.permission),
  );
  ...
  if (!hasModule('hr') && !isSuperuser) {
    return (
      <div className="p-8 text-center text-slate-400 text-sm">
        HR 모듈 라이선스가 활성화되지 않았습니다. 설정 → 모듈에서 활성화하세요.
      </div>
    );
  }
      ...
      <aside className="lg:w-56 shrink-0 border-b lg:border-b-0 lg:border-r border-slate-800 bg-slate-950/80 overflow-y-auto">
        <div className="p-3 border-b border-slate-800">
          <Link
            ...
            className="flex items-center gap-2 text-sm font-semibold text-slate-100 hover:text-cyan-300"
          >
                    ...
                    <li key={item.href}>
                      <Link
                        ...
                        className={`block text-xs px-2 py-1.5 rounded-lg transition-colors ${
                          active
                            ? 'bg-cyan-900/40 text-cyan-200 font-medium'
