...
export default function PurchaseVoucherIntegrationPanel({ onActionsChange }: Props) {
  ...
  const [pattern, setPattern] = useState<PurchaseVoucherPattern>(DEFAULT_PURCHASE_VOUCHER_PATTERN);
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/accounting/integration/purchase?${params}`, { headers });
  ...
  const processSelected = useCallback(async () => {
    ...
    try {
      const headers = await getAuthJsonHeaders();
      const res = await fetch('/api/accounting/integration/purchase', {
        method: 'POST',
        headers,
        body: JSON.stringify({
          storeId,
          ...
          <table className="w-full text-xs min-w-[960px]">
            ...
            <tbody>
              {rows.map(row => {
                ...
                const preview = pattern.lines
                  .filter(l => !pattern.splitVat ? l.amountKey !== 'tax' : true)
                  .map(l => {
                    const amt = l.amountKey === 'supply'
                      ? (row.supplyAmount || Math.max(row.totalAmount - row.taxAmount, 0))
                      : l.amountKey === 'tax'
                        ? row.taxAmount
                        : row.totalAmount;
                    if (!amt) return null;
                ...
                return (
                  <tr key={row.id} className={`border-t border-slate-800/80 ${isLinked ? 'opacity-70' : ''}`}>
                    ...
                    <td className="px-3 py-2 text-slate-300">{row.purchaseDate}</td>
                    <td className="px-3 py-2 text-slate-200">{row.supplierName || '—'}</td>
                    <td className="px-3 py-2 text-slate-400 font-mono">{row.invoiceNumber || '—'}</td>
                    <td className="px-3 py-2 text-right tabular-nums text-slate-300">{row.supplyAmount.toLocaleString()}</td>
