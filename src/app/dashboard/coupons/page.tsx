...
import { Tag, Plus, Loader2, ToggleLeft, ToggleRight, Trash2, Sparkles, ImageIcon } from 'lucide-react';
  ...
  return (
    <div className="max-w-4xl mx-auto p-6">
      ...
      {loading ? (
        ...
        <p className="text-slate-500 text-center py-12">등록된 쿠폰이 없습니다. 「AI로 만들기」로 이미지·바코드 쿠폰을 만들어 보세요.</p>
      ) : (
            ...
            <div key={c.id} className="flex items-center gap-3 bg-slate-900 border border-slate-800 rounded-xl px-4 py-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-slate-400">
                  ...
                  {' · '}사용 {c.usedCount}/{c.maxUse || '∞'}
                  {c.endDate && ` · ~${c.endDate}`}
                  {c.barcodeValue && c.barcodeValue !== c.code && ` · 바코드 ${c.barcodeValue}`}
                </p>
              </div>
              <button onClick={() => toggleActive(c)} className="text-slate-400 hover:text-teal-400">
                {c.isActive ? <ToggleRight className="w-6 h-6 text-teal-400" /> : <ToggleLeft className="w-6 h-6" />}
              </button>
              <button onClick={() => handleDelete(c)} className="text-slate-500 hover:text-red-400">
      ...
      <p className="text-xs text-slate-600 mt-6">
        AI 쿠폰: 대화로 조건 설정 → FLUX 배경 + Code128 바코드 합성 · 직접 이미지 업로드 가능<br />
        키오스크 검증: <code className="text-teal-500">/kiosk/coupon-overlay</code>
      </p>
