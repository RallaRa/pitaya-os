...
import { ArrowLeft, Percent, Loader2, Save, Check, Info } from 'lucide-react';
...
export default function CostRatioTargetsSettingsPage() {
  const { currentStore } = useStore();
  const storeId = currentStore?.storeId || '';

  const [globalTarget, setGlobalTarget] = useState(65);
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [settingsRes, itemsRes] = await Promise.all([
        fetch(`/api/store/cost-ratio-targets?storeId=${encodeURIComponent(storeId)}`, {
      ...
      setGlobalTarget(Math.round((s.globalTargetRatio ?? 0.65) * 1000) / 10);
      const itemTargets = (s.itemTargets || {}) as Record<string, number>;

      setItems(
        (itemsData.items || []).map((it: { id: string; cut?: string; name?: string; appliedCost?: number }) => ({
          id: it.id,
          name: String(it.cut || it.name || '품목'),
          appliedCost: Math.round((it.appliedCost ?? 0) * 1000) / 10,
  ...
  const save = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/store/cost-ratio-targets', {
        method: 'PUT',
        headers: await getAuthJsonHeaders(),
        body: JSON.stringify({
          storeId,
          settings: {
            globalTargetRatio: globalTarget / 100,
            itemTargets,
          },
        }),
      });
      ...
      <div className="max-w-2xl mx-auto px-4 py-6 space-y-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500 mt-0.5">전체·품목별 목표 — 초과 시 매출알림 채널 경고</p>
          </div>
        </div>
          ...
          <>
            <div className="bg-slate-900/60 border border-slate-800 rounded-xl p-4 space-y-3">
              <label className="block">
                <span className="text-sm text-slate-300">매장 전체 목표 원가율 (%)</span>
                <input
                  type="number"
                  min={10}
                  max={95}
              ...
              <p className="text-[10px] text-slate-600 flex items-center gap-1">
                <Info className="w-3 h-3" /> 개별 설정 없는 품목은 품목 마스터「적용원가율」→ 없으면 전체 목표 적용
              </p>
            </div>
              ...
              <div className="max-h-96 overflow-y-auto divide-y divide-slate-800/60">
                {items.map(it => (
                  <div key={it.id} className="flex items-center gap-3 px-4 py-2.5 text-sm">
                    ...
                    <span className="text-[10px] text-slate-600 shrink-0">마스터 {it.appliedCost}%</span>
                    <input
                      type="number"
                      min={10}
                      max={95}
            ...
            <button
              ...
              onClick={save}
              disabled={saving}
              className="w-full flex items-center justify-center gap-2 py-2.5 bg-teal-600 hover:bg-teal-500 disabled:opacity-50 text-white rounded-xl text-sm font-semibold"
            >
