...
- 날짜 확인 문구는 반드시 1개만 작성하세요. 절대 중복 작성 금지.`;
    ...
    if (msg.includes('503')) {
      ...
      userMessage = '⚠️ API 인증 오류입니다. Gemini API 키가 유효하지 않거나 만료되었습니다. .env.local 파일의 GEMINI_API_KEY를 확인해주세요.';
      retryable = false;
    } else if (msg.includes('404')) {
      userMessage = '⚠️ AI 모델을 찾을 수 없습니다. gemini-2.5-flash 모델명을 확인해주세요.';
      retryable = false;
    } else if (msg.includes('500')) {
      userMessage = '⚠️ Gemini 서버 내부 오류입니다. 구글 AI 서버에 일시적인 문제가 발생했습니다. 잠시 후 재시도해주세요.';
      retryable = true;
    } else if (msg.includes('Failed to fetch') || msg.includes('ECONNREFUSED')) {
      userMessage = '⚠️ 네트워크 연결 오류입니다. 인터넷 연결 상태를 확인해주세요.';
      retryable = true;
    } else if (msg.includes('JSON') || msg.includes('parse')) {
      userMessage = '⚠️ AI 응답 파싱 오류입니다. AI가 올바르지 않은 형식으로 응답했습니다. 다시 시도해주세요.';
      retryable = true;
    } else {
