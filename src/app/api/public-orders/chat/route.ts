...
- add_lines: 품목 추가 (lines 배열) — 사용자가 「새 품목/신규/새로 추가」라고 명시한 경우 또는 회차에 품목이 없을 때만
- update_line: 품목 수정 (lineName + lineUpdates) — 가격·수량·사진·설명 변경의 기본 수단
- remove_line: 품목 삭제 (lineName)

lines[] 필드: name(필수), description, origin, normalPrice, discountPrice, unit(주문·재고 단위, 기본 kg), priceUnitLabel(가격 표시, unit과 다를 때. 예: "100g(1팩 200~300g)"이면 unit은 "팩"), totalQty(기본 10)

규칙:
- unit = 손님이 +/- 로 주문하는 단위(팩, kg, 마리)
- priceUnitLabel = 가격 옆에만 표시(100g, 100g(1팩 200~300g) 등). 없으면 unit과 동일 표시
- "접수 시작/오픈/열어줘" → update_session status: open
- "마감/닫아줘" → update_session status: closed
- 사진이 첨부된 경우 품목·photoUrl은 서버가 자동 처리함 → add_lines에 photoUrl 넣지 말 것
- 사진·가격·수량 변경은 update_line 우선. add_lines는 「새 품목/신규/새로 추가」 명시 또는 회차 품목 0개일 때만
- 유사한 기존 품목명이 있으면 반드시 update_line(lineName=기존명) 사용, add_lines 금지
...
export async function POST(req: Request) {
  ...
  try {
    ...
    const explicitNewItems = wantsExplicitNewPublicOrderLine(message);
    ...
    if (images.length > 0) {
      const existingLineNames = context.current?.lines.map(l => l.name) || [];
      const vision = await analyzePublicOrderImages(images, message, existingLineNames);
...
[사용자 신규 품목 요청: ${explicitNewItems ? '예 (add_lines 가능)' : '아니오 — 기존 품목 update_line 우선, add_lines는 회차 품목 없을 때만'}]
...
품목: ${context.current.lines.map(l =>
    `${l.name}${l.photoUrl ? '📷' : ''} ${l.discountPrice || l.normalPrice}원/${l.priceUnitLabel || l.unit}(주문:${l.unit})`,
  ).join(' · ') || '없음'}`
  : '(선택된 회차 없음)'}
...
user: ${message || '(사진만 첨부)'}`;
    ...
    if (message || !images.length) {
      ...
      const execPhoto = photoActions.length
        ? await executePublicOrderActions(storeId, photoActions, activeAfterPhotos, { userMessage: message })
    ...
    // 사진만 전송
    const exec = await executePublicOrderActions(storeId, photoActions, activeAfterPhotos, { userMessage: message });
    const base = process.env.NEXT_PUBLIC_APP_URL || 'https://pitaya-osv1.vercel.app';
