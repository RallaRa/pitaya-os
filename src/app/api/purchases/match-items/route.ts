  ...
  try {
    ...
    if (aliasMatch) {
      return NextResponse.json({
        ...
        needConfirm: false,
        item: aliasMatch,
        message: `✅ ${itemName} → ${(aliasMatch as { normalizedName?: string; name?: string }).normalizedName || (aliasMatch as { name?: string }).name || itemName} 자동 매칭`,
      });
    }
...
export async function PUT(req: NextRequest) {
  ...
  try {
    ...
    await adminDb.collection('item_aliases').doc(key).set({
      ...
      confidence: confidence ?? 100, storeId,
...
export async function DELETE(req: NextRequest) {
  ...
  await adminDb.collection('item_aliases').doc(id).delete();
