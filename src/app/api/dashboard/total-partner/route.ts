...
export async function GET(req: Request) {
  ...
  const noAiMsg = 'AI API 키가 서버에 설정되지 않았습니다. Vercel 환경변수(GEMINI/ANTHROPIC/GROQ 등)를 확인하세요.';
  if (!hasAnyAiProvider()) {
    const periodStub = (label: string): GeminiPeriodData => ({
      period: label,
      opinion: noAiMsg,
      topItems: [],
      bottomItems: [],
    ...
    return NextResponse.json({
      ...
      today: periodStub(`오늘 ${today}(${DOW_KO[todayDow]})`),
      tomorrow: periodStub(`내일 ${tomorrowStr}(${DOW_KO[tomorrowDate.getDay()]})`),
      thisWeek: { ...periodStub(`이번주 ${weekRange}`), weekHighlight: '' },
      thisMonth: {
        ...periodStub(`이번달 ${now.getFullYear()}년 ${now.getMonth() + 1}월`),
        monthProgress: `${monthDay}일 경과 / ${daysLeft}일 남음`,
        salesForecast: '예측불가',
      },
      orderAdvice: { isOrderDay: false, dDay: null, comment: noAiMsg, items: [] },
      dataSourceStatus: {
        salesHistory: { status: 'empty' },
        weather: { status: 'error' },
        meatPrice: { status: 'error' },
        trendData: { status: 'empty' },
  ...
  fs.dailyTotals.forEach(d => {
    const compact = String(d.date || '').replace(/-/g, '');
    if (compact.length < 8) return;
    const dateObj = new Date(
      `${compact.slice(0, 4)}-${compact.slice(4, 6)}-${compact.slice(6, 8)}T12:00:00+09:00`,
    );
  ...
  const todayHeaderSale = fs.dailyTotals.find(d => d.date.replace(/-/g, '') === todayNum)?.totalSale ?? 0;
  const hasData =
    fs.topItems90.length > 0
    || fs.dailyTotals.length > 0
    || fs.closures.length > 0
    || todayHeaderSale > 0
    || (storeId ? await storeHasSalesData(storeId) : false);

  if (!hasData) {
    const missingSrc = Object.entries(dataStatus)
      .filter(([, v]) => v.status === 'error' || v.status === 'empty')
      .map(([k]) => k);
    const emptyReason = `POS·일마감 매출 이력이 없습니다. pos_sales_detail / pos_daily_sales / daily_reports 중 하나 이상 필요합니다. 미수집: ${missingSrc.slice(0, 5).join(', ') || '전체'}`;
    const fallback = {
      generatedAt: new Date().toISOString(),
      noData: true,
      emptyReason,
...
topItems/bottomItems badge는: HOT(+30%↑) | UP(+10~30%) | 주의(-10%↓) | 추천(최적조건)`;
  ...
  try {
    ...
    result = {
      generatedAt: new Date().toISOString(),
      today:     { period:`오늘 ${today}(${DOW_KO[todayDow]})`, opinion:`**AI 분석 일시 오류**입니다. 새로고침을 눌러 다시 시도하세요. (${errMsg.slice(0, 80)})`, topItems:[], bottomItems:[], keyAlert:'AI 오류', confidence:0 },
