...
데이터 수정·삭제·추가 요청은 정중히 거절하세요.`;
  ...
  const bizStatus = context.todaySales?.isClosed ? '마감완료' : '영업중';
...
영업상태: ${bizStatus}`;
  ...
  if (context.employees.length > 0) {
    prompt += `\n\n직원 (${context.employees.length}명): ${context.employees.slice(0, 10).map(e =>
      `${e.name || '이름없음'}(${e.jobPosition || '-'})`,
    ).join(', ')}`;
  }

  prompt += '\n\n위 데이터를 바탕으로 질문에 답해줘. 데이터 수정·삭제·추가 요청은 정중히 거절해.';

  return prompt;
}

export const DEBATE_SYSTEM_PROMPT = `너는 토론 진행자야. 사용자가 제시한 주제에 대해
